...
class Util
{
     ...
     * @param bool $httponly
     */
    public static function setSecureCookie($name, $value = '', $time = 0, $httponly = true, $domain = null, $secure = false)
    {
        $_COOKIE[$name] = $value ? self::encrypt($value, Config::get('SYS_KEY')) : '';
        setcookie($name, $_COOKIE[$name], $time, '/', $domain, $secure, $httponly);
    }
    ...
    {
        return isset($_COOKIE[$name]) ? self::decrypt($_COOKIE[$name], Config::get('SYS_KEY')) : '';
    }
    ...
    public static function verifyToken()
    {
        ...
        self::setSecureCookie('t', '', 0);
        return $token < $_SERVER['REQUEST_TIME'] && $token > $_SERVER['REQUEST_TIME'] - 60 * 5;
    }
    ...
    {
        return substr(md5(microtime(1)), 0, $length);
    }
     ...
     * 按概率随机弹出一个key
     *
     * @param $proArr array("first"=>20,"second"=>30,"third"=>50)
     * @return string int key
     */
    public static function chance($proArr)
    {
        $result = '';
        // 概率数组的总概率精度
     ...
     * @return string
     */
    public static function pinYin($str, $space = false, $isShort = false)
    {
        $ret = "";
    ...
    public static function arrayDistillByKey($arr, $keyKey, $valKey = '')
    {
        $set = [];
        foreach ($arr as $subArr) {
            if ($valKey) {
                $set[$subArr[$keyKey]] = $subArr[$valKey];
            } else {
                $set[] = $subArr[$keyKey];
            }
        }
